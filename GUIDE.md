...
1. В текущей сборке png\jpg автоматически не ужимаются, webp не создается.
...
4. Добавлена **[компонентная сущность модалок](#Модалки)**
5. Обновлена файлова структура, добавлен пример `container.scss`, в `reboot.scss` собраны различные ресеты \ кроссбраузерные фиксы

## Gulp-file-include
Так как в текущее время разработка большинства проектов связана с созданием копонентов - возникла необходимость в инклюдах передавать переменные. Ведь в стандартном `posthtml-include` расширить компонент модификатором - приходилось создавать новый html файл, копипастить содержимое. И при изменении базы компонента - приходилось бы редактировать и модифицированную версию, а это нарушает принцип компонентного подхода.
...
```html
    <include src="source/html/base/head.html"></include>
```
...
```
  @@include("source/html/base/card.html", {
    "title": "text",
    "mod": "tall"
  })
```
...
2. Глобальное
Глобальное объявление переменных доступно только через gulp, объявить глобальную переменную на страницу невозможно.
Глобальные переменные содержатся в объекте context. 
...
Проблема может возникнуть, если компонент на одной странице передается с переменной, а на другой без.
...
❗ Нельзя использовать в объекте переменных ключи с дефисом `test-1`, если в компоненте вы их вызываете через `context.test-1`

### Циклы.
Циклы в этом плагине очень недружелюбны, по ним нет адекватной информации в спецификации. В 99.9% случаев если на проекте необходимы циклы и такого рода компоненты - лучше использовать `pug`. Но все же полезно будет узнать, что еще умеет этот плагин.
...
+ в gulp необходимо задать глобальные переменные, чтобы не копипастить их на каждую страницу, что нарушает логику компонентности
...
В js необходимо найти модалку и ссылки на нее и передать как аргументы в `setupModal()`.
...
## Для серьезных проектов имеет смысл немного изменить подход в создании переменных.
...
### Если вы не знакомы с подходами HtmlAcademy в работе с gulp или вам не до конца понятна как работает сборка - предлагаю ознакомиться с дополнительным [гайдом по работе с gulp](/gulp-guide) 📘
